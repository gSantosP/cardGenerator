...
if(valorNome == "" || valorDescricao == "" || valorImg == ""){
    ...
    var storagechecker = function(){

        let storage = JSON.parse(sessionStorage.getItem("cards"))
    ...
    armazem_pre.push(card_info)

    sessionStorage.setItem('cards', JSON.stringify(armazem_pre))
...
window.onload = function aplicaArmazenadas(){
    let armazem = JSON.parse(sessionStorage.getItem('cards'))

    console.log(armazem)
    for(card of armazem){
        criaArmazenadas(card.nome,card.imagem,card.desc)
    }
}

// "criaArmazenadas" tá em outro arquivo js chamado modulo.js
